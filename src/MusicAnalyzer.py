        ...
        for country, df in top_50_by_country.items():
            for artist_list in df['Artists']:
                for artist in [a.strip() for a in artist_list.split(',')]:
                    artist_country_map.setdefault(artist, set()).add(country)
        data = [{'Artist': a, 'Country Count': len(c)} for a, c in artist_country_map.items()]
        df = pd.DataFrame(data).sort_values(by=['Country Count', 'Artist'], ascending=[False, True]).reset_index(drop=True)
        return df.head(50)
